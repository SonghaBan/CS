...
#==============================================================================
            if code == M_HANGMAN:
                ...
                if to_name == from_name:
                    msg = M_HANGMAN + 'Hey there'
                elif self.group.is_member(to_name):
                    g, w = self.group.find_group(to_name)
                    if not g:
                        msg = M_HANGMAN + 'ok'
                        mysend(self.logged_name2sock[to_name], M_REQUEST + from_name)
                else:
                    msg = M_HANGMAN + 'no_user'
                mysend(from_sock, msg)
            
            elif code == M_GUESSING:
                from_name = self.logged_sock2name[from_sock]
                ...
                for g in the_guys[1:]:
                    ...
                    self.indices[g].add_msg_and_index(said2)
                    mysend(to_sock, msg)
                mysend(to_sock, M_WORDPICKER + msg)
                
            
            elif code == M_RESPONSE:
                to_name = msg[2:].strip() #gets to_name from response
                if msg[1] == 'Y':
                    from_name = self.logged_sock2name[from_sock]
                    self.group.connect(from_name, to_name)
                    to_sock = self.logged_name2sock[to_name]
                    mysend(from_sock, M_WORDPICKER + to_name)
                    
                elif msg[1] == 'N':
                    from_name = self.logged_sock2name[from_sock]
                    to_sock = self.logged_name2sock[to_name]
                    mysend(to_sock, M_DECLINE + from_name)
...
#==============================================================================
                ...
                poem = '\n'.join(poem)
                print("Requested poem:" + poem)               
                mysend(from_sock, M_POEM + poem)
#==============================================================================
#             retrieve the time; IMPLEMENT THIS
#==============================================================================
            elif code == M_TIME:
                ctime = time.strftime('%m.%d.%y,%H:%M', time.localtime())
...
#==============================================================================
                ...
                term = msg[1:]
                from_name = self.logged_sock2name[from_sock]                
                search_rslt=str(self.indices[from_name].search(term)).strip()
                print("Search Result:" + search_rslt)
